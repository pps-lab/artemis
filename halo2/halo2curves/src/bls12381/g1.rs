...
use crate::{batch_add, new_curve_impl, serde, CurveAffineExt};
...
impl crate::serde::SerdeObject for G1Projective {
    fn from_raw_bytes_unchecked(bytes: &[u8]) -> Self {
        debug_assert_eq!(bytes.len(), 3 * Fp::size());
        let [x, y, z] = [0, 1, 2]
            .map(|i| Fp::from_raw_bytes_unchecked(&bytes[i * Fp::size()..(i + 1) * Fp::size()]));
        Self { x, y, z }
    }
    fn from_raw_bytes(bytes: &[u8]) -> Option<Self> {
        if bytes.len() != 3 * Fp::size() {
            return None;
        }
        let [x, y, z] =
            [0, 1, 2].map(|i| Fp::from_raw_bytes(&bytes[i * Fp::size()..(i + 1) * Fp::size()]));
    ...
    fn to_raw_bytes(&self) -> Vec<u8> {
        let mut res = Vec::with_capacity(3 * Fp::size());
        Self::write_raw(self, &mut res).unwrap();
        res
    }
    fn read_raw_unchecked<R: std::io::Read>(reader: &mut R) -> Self {
...
fn test_clear_cofactor() {
    ...
    let h_eff = Scalar::from(1) + Scalar::from(crate::bls12381::BLS_X);
