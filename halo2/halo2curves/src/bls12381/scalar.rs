...
impl crate::serde::SerdeObject for Scalar {
...
fn test_zeta() {
    ...
    println!("TWO : {:?},\nTWO_INV : {:?},\nTWO : {:?},\nTWO_INV : {:?},", TWO_INV.invert().unwrap(), TWO_INV, Scalar::from(2u64), Scalar::from(2u64).invert().unwrap());
    println!("Generator: {:?}", GENERATOR);
    println!("ROOT_OF_UNITY: {:?}", ROOT_OF_UNITY);
    println!("ROOT_OF_UNITY_INV: {:?}", ROOT_OF_UNITY.invert().unwrap());
    println!("ROOT_OF_UNITY_INV_2: {:?}", ROOT_OF_UNITY_INV);
    assert_eq!(Scalar::from(1), zeta * zeta);
}
