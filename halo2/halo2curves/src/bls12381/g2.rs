...
impl crate::serde::SerdeObject for G2Affine {
    fn from_raw_bytes_unchecked(bytes: &[u8]) -> Self {
        debug_assert_eq!(bytes.len(), 2 * Fp2::size());
        let [x, y] =
            [0, Fp2::size()].map(|i| Fp2::from_raw_bytes_unchecked(&bytes[i..i + Fp2::size()]));
        Self { x, y, infinity: Choice::from(0u8) }
    }
    fn from_raw_bytes(bytes: &[u8]) -> Option<Self> {
        if bytes.len() != 2 * Fp2::size() {
            return None;
        }
        let [x, y] = [0, Fp2::size()].map(|i| Fp2::from_raw_bytes(&bytes[i..i + Fp2::size()]));
        x.zip(y).and_then(|(x, y)| {
            let res = Self { x, y, infinity: Choice::from(0u8)};
            // Check that the point is on the curve.
            bool::from(res.is_on_curve()).then(|| res)
        })
    }
    fn to_raw_bytes(&self) -> Vec<u8> {
        let mut res = Vec::with_capacity(2 * Fp2::size());
        Self::write_raw(self, &mut res).unwrap();
        res
    }
    fn read_raw_unchecked<R: std::io::Read>(reader: &mut R) -> Self {
        let [x, y] = [(); 2].map(|_| Fp2::read_raw_unchecked(reader));
        Self { x, y, infinity: Choice::from(0u8) }
    }
    fn read_raw<R: std::io::Read>(reader: &mut R) -> std::io::Result<Self> {
        let x = Fp2::read_raw(reader)?;
        let y = Fp2::read_raw(reader)?;
        Ok(Self { x, y, infinity: Choice::from(0u8)})
    }
...
impl G2Projective {
    ...
    fn mul_by_x(&self) -> G2Projective {
        ...
        let mut x = crate::bls12381::BLS_X >> 1;
        let mut acc = *self;
        while x != 0 {
            acc = acc.double();
            if x % 2 == 1 {
        ...
        if crate::bls12381::BLS_X_IS_NEGATIVE {
            xself = -xself;
        }
        xself
    }
...
fn test_mul_by_x() {
    ...
    let x = if crate::bls12381::BLS_X_IS_NEGATIVE {
        -Scalar::from(crate::bls12381::BLS_X)
    } else {
        Scalar::from(crate::bls12381::BLS_X)
    };
...
fn test_endo_consistency() {
    let g = G2Projective::generator();
    assert_eq!(g * Scalar::ZETA, g.endo());
}
