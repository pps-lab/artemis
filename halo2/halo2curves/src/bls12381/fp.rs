...
use crate::bls12381::util::{adc, mac, sbb};
...
const ROOT_OF_UNITY_INV: Fp = TWO_ADIC_ROOT_OF_UNITY;
...
impl PrimeField for Fp {
    ...
    const S: u32 = S;
    const ROOT_OF_UNITY: Self = TWO_ADIC_ROOT_OF_UNITY;
...
impl Fp {
    ...
    pub(crate) const fn montgomery_reduce_new(
        t0: u64,
        t1: u64,
        t2: u64,
        t3: u64,
        ...
        (&Fp([r6, r7, r8, r9, r10, r11])).sub(&MODULUS)
    }
    #[inline]
    ///
...
#[test]
fn test_field() {
    crate::tests::field::random_field_tests::<Fp>("fp".to_string());
}
...
fn test_zeta() {
    ...
    println!("One: {:?}", R);
    println!("TWO : {:?},\nTWO_INV : {:?},\nTWO : {:?},\nTWO_INV : {:?},", TWO_INV.invert().unwrap(), TWO_INV, Fp::from(2u64), Fp::from(2u64).invert().unwrap());
    println!("Zeta: {:?}", ZETA);
    println!("-1: {:?}, \n -2: {:?}", -Fp::from(1), -Fp::from(2));
    println!("Last test: {:?}", -Fp::from_raw([
        0x5360_bb59_7867_8032,
        0x7dd2_75ae_799e_128e,
        0x5c5b_5071_ce4f_4dcf,
        0xcdb2_1f93_078d_bb3e,
    ...
    println!("ROOT_OF_UNITY: {:?}", TWO_ADIC_ROOT_OF_UNITY);
    println!("ROOT_OF_UNITY_INV: {:?}", TWO_ADIC_ROOT_OF_UNITY.invert().unwrap());
    println!("ROOT_OF_UNITY_INV_2: {:?}", ROOT_OF_UNITY_INV);
    println!("(T-1) / 2: {:?}", T_MINUS_ONE_DIV_TWO);
    println!("(T-1) / 2: {:?}", (T_MINUS_ONE_DIV_TWO * Fp::from(2) + Fp::from(1)) * Fp::from(2) + Fp::from(1));
    assert_eq!(Fp::one(), zeta * zeta);
}
