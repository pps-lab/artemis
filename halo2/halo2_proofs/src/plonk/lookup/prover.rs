...
use ff::{WithSmallOrderMulGroup, PrimeField};
use group::{
...
fn permute_expression_pair<'params, C: CurveAffine, P: Params<'params, C>, R: RngCore>(
    ...
    let mut leftover_table_map: HashMap<ScalarWrapper<C>, u32> = HashMap::new();
    for &coeff in table_expression.iter().take(usable_rows) {
    ...
    for (coeff, count) in leftover_table_map.iter() {
        for _ in 0..*count {
            permuted_table_coeffs[repeated_input_rows.pop().unwrap() as usize] = coeff.value;
        }
    }
