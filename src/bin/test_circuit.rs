...
fn main() {
  ...
  let witness_column = std::env::args().nth(3).expect("input file path").parse().unwrap();

  let config: ModelMsgpack = load_model_msgpack(&config_fname, &inp_fname, witness_column);

  let circuit = ModelCircuit::<Fr>::generate_from_file(&config_fname, &inp_fname, witness_column, 7);
