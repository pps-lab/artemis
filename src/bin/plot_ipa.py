...
def get_prover_time_ipa (path):
    ...
    for line in lines:
        if (line.startswith("IPA commit proof time")):
            res = round(float(line.split(" ")[-1][:-2]), 1)
...
# nocom results
for name in names:
    filepath = os.path.join('logs', name + '_nocom_ipa.txt')
    prover_times_nocom.append(get_prover_time(filepath))
    verifier_times_nocom.append(get_verifier_time(filepath))

# poly results
for name in names:
    filepath = os.path.join('logs', name + '_poly_ipa.txt')
    prover_times_poly.append(get_prover_time(filepath))
    verifier_times_poly.append(get_verifier_time(filepath))

for name in names:
    filepath = os.path.join('logs', name + '_nocom_ipa.txt')
    prover_times_ipa.append(get_prover_time_ipa(filepath))
    verifier_times_nocom.append(get_verifier_time(filepath))
# # pos results
# for name in names:
...
prover_times_ipa = [0.2, 1, 3, 7.3, 42, 25, 193, 300]
# cplink = []
# cplink_lite = []
names_sizes = []
prover_times_full = []
for (time, ipa) in zip(prover_times_poly, prover_times_ipa):
    print (time)
    prover_times_full.append(time + ipa)
...
poly = ax.bar(x, prover_times_full, width=bar_width, label='Poly circuit', color='orange', alpha = 1)
nocom = ax.bar(x + bar_width, prover_times_nocom, width=bar_width, label='No commitment', color='skyblue', alpha = 1)
...
plt.savefig(time + 'ipa' + '.svg', format='svg')
