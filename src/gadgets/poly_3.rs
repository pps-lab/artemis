...
pub struct Poly3Chip<F: PrimeField> {
...
impl<F: PrimeField> Poly3Chip<F> {
  ...
  pub fn configure(meta: &mut ConstraintSystem<F>, gadget_config: GadgetConfig) -> GadgetConfig {
    ...
    meta.create_gate("Poly gate", |meta| {
      ...
      let gate_inp = meta.query_advice(Poly3Chip::<F>::get_beta_columns(columns), Rotation::cur());
      let gate_coeffs =  Poly3Chip::<F>::get_coeff_columns(columns)
        .iter()
...
impl<F: PrimeField> Gadget<F> for Poly3Chip<F> {
  fn name(&self) -> String {
    "Poly".to_string()
  }
  ...
  fn op_row_region(
    ...
    let coeff_cols = Poly3Chip::<F>::get_coeff_columns(&cols);
    let alpha_cols = Poly3Chip::<F>::get_alpha_columns(&cols);
    let beta_cols = Poly3Chip::<F>::get_beta_columns(&cols);
    ...
    //   .iter()
    //   .enumerate()
    //   .map(|(i, cell)| cell.copy_advice(|| "", region, input_cols, row_offset))
    //   .collect::<Result<Vec<_>, _>>()
    //   .unwrap();
    ...
    let beta = region
      .assign_advice(
        ...
        beta_cols,
        row_offset,
        || Value::known(self.betas[1]),
      )
      .unwrap();

    let alphas = self.alphas.iter().enumerate().map(| (idx, alpha)| {
      region
      .assign_advice(
        || "",
        alpha_cols[idx],
        row_offset,
        || Value::known(self.alphas[idx]),
      )
      .unwrap()
    }).collect::<Vec<_>>();
    ...
    let mut e = self.alphas.iter().zip(coeffs.iter())
      .map(|(a, b)|  Value::known(*a) * b.value())
      .reduce(|a, b| a + b)
      .unwrap();
    e = e + beta.value().map(|x| *x) * bias.value().map(|x| *x); //beta_vec.iter()
      // .zip(coeffs.iter())
      // .map(|(a, b)|  Value::known(*a) * b.value())
      // .reduce(|a, b| a + b)
      // .unwrap();
