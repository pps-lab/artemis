...
impl<F: PrimeField> Poly2Chip<F> {
  ...
  pub fn get_weight_columns(columns: &Vec<Column<Advice>>) -> Vec<Column<Advice>> {
    let num_inputs = (columns.len() - 2) / 2;
  ...
  pub fn configure(meta: &mut ConstraintSystem<F>, gadget_config: GadgetConfig) -> GadgetConfig {
    ...
    meta.create_gate("Poly gate", |meta| {
      ...
      let gate_inp = Poly2Chip::<F>::get_input_columns(columns)
        ...
        .collect::<Vec<_>>();
      let gate_weights = Poly2Chip::<F>::get_weight_columns(columns)
        .iter()
        .map(|col| meta.query_advice(*col, Rotation::cur()))
        .collect::<Vec<_>>();
...
impl<F: PrimeField> Gadget<F> for Poly2Chip<F> {
  ...
  fn op_row_region(
    ...
    let weight_cols = Poly2Chip::<F>::get_weight_columns(&cols);
    weights
      .iter()
      .enumerate()
