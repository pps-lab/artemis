...
class BrokenColumnCrossPlotLoader(BaseColumnCrossPlotLoader): 
        def plot(self, df: pd.DataFrame) -> List:
            if df.empty:
                return
            ...
            for _i_plot, df_plot, plot_id in self.fig_foreach.for_each(df, parent_id={}):
                ...
                for subplot_idx, df_subplot, subplot_id in subplot_grid.for_each(
                    ...
                    #ax = axs[subplot_idx]
                    ax_top = axs[(0, 2), ][subplot_idx]
                    ax_bottom = axs[(1, 3), ][subplot_idx]
                    ax = axs[subplot_idx]
                    ...
                    print(f"  subplot_id={data_id} \ndf_subplot={df_subplot['prover_time_sec'].iloc[0]}")
                    max_val = df_subplot['prover_time_sec'].max()
                    min_val = df_subplot['prover_time_sec'].min()
                    ax_top.set_ylim(max_val*0.7, max_val*1.1)
                    ax_bottom.set_ylim(0, min_val*3)
