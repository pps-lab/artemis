...
class GroupedStackedBarChart(MyETLBaseModel):
    ...
    def plot(
        ...
        for (
            ...
            # Add value label
            #if part_value is not None and part_value != 0:
                # #ax.annotate(
                #     f"{part_value:.2f}",
                #     xy=(position.bar_center_pos, position.bar_part_bottom + part_value),
                #     xytext=(0, 3),
    ...
    def for_each(self, df1: pd.DataFrame, metric: Metric, subplot_id: Dict[str, str]):
        ...
        for i_group, df_bar_group, bar_group_id in self.group_foreach.for_each(
            ...
            print(f"{df_bar_group['cpsnark']=}")
            required_cpsnarks = ['no_com', 'poly', 'cp_link_fast', 'poseidon', 'zkfft']
            # Step 1: Add missing cpsnarks
            existing = set(df_bar_group['cpsnark'])
            missing = [c for c in required_cpsnarks if c not in existing]

            # Add rows with only 'cpsnark' set, rest are filled with 0
            default_row = {col: 0 for col in df_bar_group.columns if col != 'cpsnark'}
            new_rows = pd.DataFrame([{**default_row, 'cpsnark': c} for c in missing])

            df_bar_group = pd.concat([df_bar_group, new_rows], ignore_index=True)

            # Step 2: Reorder
            df_bar_group['cpsnark'] = pd.Categorical(df_bar_group['cpsnark'], categories=required_cpsnarks, ordered=True)
            df_bar_group = df_bar_group.sort_values('cpsnark').reset_index(drop=True)
...
def calc_positions(
    ...
    for _i_group, df_bar_group, bar_group_id in group_foreach.for_each(
        ...
        # Create missing rows
        missing = [c for c in required_cpsnarks if c not in existing]
        new_rows = pd.DataFrame({'cpsnark': missing})
        df_bar_group = pd.concat([df_bar_group, new_rows], ignore_index=True)
