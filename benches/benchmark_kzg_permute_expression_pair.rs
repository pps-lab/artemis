...
use halo2_proofs::halo2curves::bn256::{G1Affine, Fr};
...
fn simulate_permute_expression_pair(k: i32) -> Result<Fr, Error> {
    let params_n = 1<<k;
    let usable_rows = params_n as usize;
    let mut rng = OsRng;

    let mut permuted_input_expression: Vec<Fr> = (0..params_n)
    ...
    //let mut leftover_table_map: BTreeMap<Fr, u32> = permuted_input_expression
    //    .iter()
    //    .take(usable_rows)
    ...
    let mut leftover_table_map: HashMap<ScalarWrapper<G1Affine>, u32> = HashMap::new();
    ...
    let mut permuted_table_coeffs = vec![Fr::ZERO; usable_rows];
    ...
    Ok(Fr::one() + &permuted_input_expression[0])
}
...
criterion_group!{
    ...
    targets=bench_kzg_permute_expression_pair
}
